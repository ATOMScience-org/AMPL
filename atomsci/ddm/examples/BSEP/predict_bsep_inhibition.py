...
# Example script for running BSEP inhibition classification models against user-provided data.
...
def predict_activity(args):
    ...
    if args.id_col not in colnames:
        input_df['compound_id'] = ['compound_%d' % i for i in range(input_df.shape[0])]
    ...
    if has_activity:
        ...
        pred_vals = pred_df['%s_pred' % args.activity_col].values
        conf_matrix = metrics.confusion_matrix(actual_vals, pred_vals)
        roc_auc = metrics.roc_auc_score(actual_vals, pred_vals)
...
def parse_params():
    ...
    parser.add_argument(
        '--model_type', '-m', type=str, dest='model_type', choices=['random', 'scaffold'],
    ...
    parser.add_argument(
        '--dont_standardize', '-s', type=bool, dest='dont_standardize', action='store_true',
