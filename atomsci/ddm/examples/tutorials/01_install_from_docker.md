...
After that, please follow the instruction from [Start a Docker Container](#start-a-docker-container) step.
...
To build a docker image, first clone **[AMPL](https://github.com/ATOMScience-org/AMPL)**  github repo, then find the Dockerfile:
...
cd AMPL/docker                        # Find the Dockerfile from AMPL/docker directory
```
...
To see what images are running on your machine, type "docker images". Type "docker run ... <IMAGE>" command to start:
...
To launch Notebook Server from a container, you can choose to run from a "jupyter-notebook" or "jupyter-lab" using the same port when you start the container earlier. We use "jupyter-notebook" command as examples here:
...
### To Create/Open the Jupyter Notebook UI From a Browser
...
To use the AMPL tutorials, here's a screenshot of the path:

![directory_tree](../../docs/source/_static/img/01_install_from_docker_files/tutorial_tree.png)

### Use **atomsci-env** as the Run Kernel

To use AMPL Framework, please select "atomsci-env" as the run kernel. 

* Open an existing notebook, top menu bar "Kernel" > "Change Kernel" > "atomsci-env"

![Select a kernel from a notebook](../../docs/source/_static/img/01_install_from_docker_files/docker-kernel-inside-nb.png)

* To set a run kernel from a new notebook, click "File" > "New" dropdown, and select **atomsci-env** as the run kernel
