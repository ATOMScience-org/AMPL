...
def parse_args():
    ...
    parser.add_argument('response_cols', help='Response column for source dataset. Comma separated string')
...
if __name__ == '__main__':
    ...
    response_cols = [s.strip() for s in args.response_cols.split(',')]

    df = pd.read_csv(args.csv, dtype={args.id_col:str})

    split_a = pd.read_csv(args.split_a, dtype={'cmpd_id':str})
    ss = SplitStats(df, split_a, smiles_col=args.smiles_col, id_col=args.id_col, response_cols=response_cols)
    ss.make_all_plots(dist_path=os.path.join(args.output_dir, 'split_a'))


    split_b = pd.read_csv(args.split_b, dtype={'cmpd_id':str})
    ss = SplitStats(df, split_b, smiles_col=args.smiles_col, id_col=args.id_col, response_cols=response_cols)
